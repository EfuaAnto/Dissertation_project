...
if (!$user_id) {
    echo (["success" => false, "error" => "User ID is required and cannot be null."]);
    exit;
}
...
//function WeightFluctuationStatus($user_id, $email, $weight_fluctuation){
...
//if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    //$sql = "SELECT u.user_id, u.email, p.inactivity_reminder, p.weight_fluctuation
    ...
    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
            $preferences = [
                ...
                //'weight_fluctuation' => $row['weight_fluctuation']
           ...
           // WeightFluctuationStatus($row['user_id'], $row['email'], $row['weight_fluctuation']);
